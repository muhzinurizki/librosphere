...
Route::get('/katalog/{book}', function (Book $book) {
  // Load relasi agar data kategori dan ketersediaan stok muncul
  $book->load(['category', 'items']);

  return Inertia::render('Katalog/Show', [
    'book' => $book,
    'available_count' => $book->items()->where('status', 'available')->count(),
  ]);
})->name('katalog.show');
